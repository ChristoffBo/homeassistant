#!/usr/bin/env bash
CONFIG_PATH=/data/options.json

# Load configuration
export REPO_SOURCE=$(jq -r '.repo_source' $CONFIG_PATH)
export ENABLE_NOTIFICATIONS=$(jq -r '.enable_notifications' $CONFIG_PATH)
export GOTIFY_URL=$(jq -r '.gotify_url' $CONFIG_PATH)
export GOTIFY_TOKEN=$(jq -r '.gotify_token' $CONFIG_PATH)
export GITEA_API_URL=$(jq -r '.gitea_api_url' $CONFIG_PATH)
export GITEA_TOKEN=$(jq -r '.gitea_token' $CONFIG_PATH)
export GITHUB_TOKEN=$(jq -r '.github_token' $CONFIG_PATH)
export GITLAB_TOKEN=$(jq -r '.gitlab_token' $CONFIG_PATH)
export REPO_PATH=$(jq -r '.repo_path' $CONFIG_PATH)
export REPO_BRANCH=$(jq -r '.repo_branch' $CONFIG_PATH)
export REPO_OWNER=$(jq -r '.repo_owner' $CONFIG_PATH)
export REPO_NAME=$(jq -r '.repo_name' $CONFIG_PATH)
export UPDATE_MODE=$(jq -r '.update_mode' $CONFIG_PATH)
export TIMEOUT=$(jq -r '.timeout' $CONFIG_PATH)
export LOG_LEVEL=$(jq -r '.log_level' $CONFIG_PATH)
export VALIDATE_SSL=$(jq -r '.validate_ssl' $CONFIG_PATH)
export DRY_RUN=$(jq -r '.dry_run' $CONFIG_PATH)

# Set default values if empty
[ -z "$REPO_OWNER" ] && export REPO_OWNER="alexbelgium"
[ -z "$REPO_NAME" ] && export REPO_NAME="hassio-addons"

# Debug output if log level is debug
if [ "$LOG_LEVEL" = "debug" ]; then
    echo "==== Configuration Debug ===="
    echo "REPO_SOURCE: $REPO_SOURCE"
    echo "ENABLE_NOTIFICATIONS: $ENABLE_NOTIFICATIONS"
    echo "GOTIFY_URL: $GOTIFY_URL"
    echo "GITEA_API_URL: $GITEA_API_URL"
    echo "REPO_PATH: $REPO_PATH"
    echo "REPO_BRANCH: $REPO_BRANCH"
    echo "REPO_OWNER: $REPO_OWNER"
    echo "REPO_NAME: $REPO_NAME"
    echo "============================="
fi
