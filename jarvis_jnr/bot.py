...
def get_upcoming_movies(days=7):
    try:
        ...
        url = f"{RADARR_URL}/api/v3/calendar?start={start}&end={end}&apikey={RADARR_KEY}"  # FIXED: apikey
...
def get_upcoming_episodes(days=7):
    try:
        ...
        url = f"{SONARR_URL}/api/v3/calendar?start={start}&end={end}&apikey={SONARR_KEY}"  # already correct
...
def get_movie_count():
    try:
        url = f"{RADARR_URL}/api/v3/movie?apikey={RADARR_KEY}"  # FIXED: apikey
        return len(requests.get(url, timeout=10).json())
    except: return 0

def get_series_count():
    try:
        url = f"{SONARR_URL}/api/v3/series?apikey={SONARR_KEY}"  # correct
        return len(requests.get(url, timeout=10).json())
    except: return 0
