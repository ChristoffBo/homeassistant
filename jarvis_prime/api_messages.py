...
# ======================= AEGISOPS API (ADD-ONLY) ============================
# This block wires up /api/aegisops/* endpoints used by the UI.
# It does not touch existing routes. Safe to paste at the end of this file.

import os, json, sqlite3, re
from pathlib import Path

try:
    import requests  # used to forward run-once to the internal runner
except Exception:
    requests = None
...
try:
    from aiohttp import web
except Exception:  # if this module isn't aiohttp-based, just no-op safely
    web = None

AEGIS_BASE = Path("/share/jarvis_prime/aegisops")
PB_DIR = AEGIS_BASE / "playbooks"
DB_PATH = AEGIS_BASE / "db" / "aegisops.db"
INV_PATH = AEGIS_BASE / "inventory.ini"
SCH_PATH = AEGIS_BASE / "schedules.json"

for p in [AEGIS_BASE, PB_DIR, DB_PATH.parent]:
    try: p.mkdir(parents=True, exist_ok=True)
    except Exception: pass

SAFE_NAME = re.compile(r"^[A-Za-z0-9_.-]+$")  # filenames like my_playbook.yml

def _json(data, status=200):
    return web.json_response(data, status=status)

def _text(data, status=200, ctype="text/plain"):
    return web.Response(text=str(data), status=status, headers={"Content-Type": ctype})

# --------- PLAYBOOKS ---------
async def aeg_list_playbooks(request):
    try:
        items = sorted([p.name for p in PB_DIR.glob("*.yml")] + [p.name for p in PB_DIR.glob("*.yaml")])
        return _json({"items": items})
    except Exception as e:
        return _json({"items": [], "error": str(e)}, status=500)

async def aeg_get_playbook(request):
    name = request.query.get("name","").strip()
    if not SAFE_NAME.match(name): return _json({"error":"bad name"}, status=400)
    p = PB_DIR / name
    if not p.exists(): return _json({"error":"not found"}, status=404)
    try:
        return _text(p.read_text(encoding="utf-8"))
    except Exception as e:
        return _json({"error":str(e)}, status=500)

async def aeg_save_playbook(request):
    try:
        body = await request.json()
    except Exception:
        return _json({"error":"bad json"}, status=400)
...
async def aeg_delete_playbook(request):
...
# --------- INVENTORY (ini) ---------
async def aeg_get_inventory(request):
    try:
        if not INV_PATH.exists(): return _text("[all]\n")
        return _text(INV_PATH.read_text(encoding="utf-8"))
    except Exception as e:
        return _json({"error":str(e)}, status=500)

async def aeg_put_inventory(request):
    try:
...
def _read_schedules():
    try:
        ...
        return json.loads(SCH_PATH.read_text(encoding="utf-8") or "[]")
    except Exception:
        return []

def _write_schedules(rows):
    SCH_PATH.parent.mkdir(parents=True, exist_ok=True)
    SCH_PATH.write_text(json.dumps(rows, indent=2), encoding="utf-8")

async def aeg_list_schedules(request):
    return _json(_read_schedules())

async def aeg_save_schedule(request):
    try:
        sch = await request.json()
    except Exception:
        return _json({"error":"bad json"}, status=400)
    if not sch.get("id"): return _json({"error":"missing id"}, status=400)
    rows = _read_schedules()
    # upsert by id
    rows = [r for r in rows if r.get("id") != sch["id"]] + [sch]
    try:
        _write_schedules(rows)
        return _json({"ok": True})
    except Exception as e:
        return _json({"error":str(e)}, status=500)

async def aeg_delete_schedule(request):
...
# --------- RUNS (sqlite) ---------
async def aeg_list_runs(request):
    ...
    try:
        ...
        cur = con.cursor()
        cur.execute("""
            SELECT ts, playbook, status, ok_count, changed_count, fail_count, unreachable_count
            FROM ansible_runs
            ORDER BY ts DESC
            LIMIT ?
        """, (limit,))
        rows = [{
            "ts": r[0], "playbook": r[1], "status": r[2],
            "ok_count": r[3], "changed_count": r[4],
            "fail_count": r[5], "unreachable_count": r[6]
...
# --------- RUN-ONCE trigger ----------
async def aeg_run_once(request):
    try:
        body = await request.json()
    except Exception:
        return _json({"error":"bad json"}, status=400)
    if not requests:
        return _json({"error":"requests module missing"}, status=500)
    try:
        # Forward to the bot’s internal endpoint the runner exposes
        r = requests.post(
            "http://127.0.0.1:2599/internal/aegisops/run",
            json={
                "playbook": body.get("playbook"),
                "servers": body.get("servers", []),
                "forks": int(body.get("forks", 1))
            },
            timeout=8
        )
        ok = (200 <= r.status_code < 300)
        return _json({"ok": ok, "code": r.status_code})
    except Exception as e:
        return _json({"error":str(e)}, status=500)

# --------- ROUTE REGISTRATION ----------
def register_aegisops_routes(app):
    if web is None:  # not an aiohttp app; nothing to do
        return
    app.router.add_get   ("/api/aegisops/playbooks",           aeg_list_playbooks)
    app.router.add_get   ("/api/aegisops/playbook",            aeg_get_playbook)
    app.router.add_post  ("/api/aegisops/playbook",            aeg_save_playbook)
    app.router.add_delete("/api/aegisops/playbook/{name}",     aeg_delete_playbook)
