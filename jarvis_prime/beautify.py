# /app/beautify.py
# Jarvis Prime – Beautify Engine (Card Bulletizer, Persona Overlay, Verify, Image-Preserving)
# 8 Stages per roadmap:
...
# 4) Rebuild structure → Jarvis-style card with *bulletized facts* (no raw dump).
# 5) Overlay persona → inject active persona voice/emoji/quip from personality.py.
# 6) Double-check vs original → verify critical info; if missing, retry relaxed.
# 7) Fail-safe → if still missing, mark Beautify Failed and push raw message.
# 8) Final render → images/posters preserved; hero set; extras carry all images + verify meta.
#
...
# ---------------------------
# Stage 4: Rebuild (strict bulletizer)
# ---------------------------
def _harvest_timestamp(text: str, title: str) -> Optional[str]:
    for src in (text or "", title or ""):
        for rx in (TS_RE, RFC2822_DT_RE, DATE_ONLY_RE, TIME_ONLY_RE):
            m = rx.search(src)
...
def _bulletize(kind: str, title: str, body: str) -> List[str]:
    # Facts
    facts: List[str] = []
    ...
    info = _first_nonempty_line(body)
    if info and info != subj: facts.append(_kv("Info", info))

    # Build bullet list from KV + signal lines; avoid dumping raw text
    bullets: List[str] = []
    ...
    # Fallback: if no bullets at all, bulletize non-empty lines (limited)
    if not bullets:
        for ln in (body or "").splitlines():
            t = ln.strip()
            if t:
    ...
    if bullets: lines += ["", _section("Facts"), *_dedup_lines(bullets)]
    return lines

# ---------------------------
# Persona overlay (Stage 5)
# ---------------------------
...
def _apply_persona(text: str, persona: Optional[str], persona_quip: bool) -> str:
    ...
    overlay_lines = [prefix] if prefix else []
    if quip_line: overlay_lines.append(quip_line)
    new_lines = lines[:insert_at] + overlay_lines + lines[insert_at:]
    return "\n".join(new_lines)

# ---------------------------
# Stage 6: Verify (two passes)
# ---------------------------
...
# ---------------------------
# Stage 7/8: Finalize (fail-safe + render + images)
# ---------------------------
...
def beautify_message(title: str, body: str, *, mood: str = "neutral",
    ...
    # Stage 4
    lines = _bulletize(kind, title, norm1)
    ...
    if missing:
        ...
        lines2 = _bulletize(kind, title, norm2)
        text1 = "\n".join(_dedup_lines(lines2)).strip()
    ...
    # Stage 8
    images = _collect_images(body)  # keep images inline and also set hero
    extras: Dict[str, Any] = {
