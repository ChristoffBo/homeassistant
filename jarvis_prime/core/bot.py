...
async def _internal_emit(request):
    ...
    source = str(data.get("source") or "internal").strip()
    priority = int(data.get("priority") or 5)

    # forward into core send
    send_message(title, body, priority=priority, extras={"source": source})
...
async def _internal_wake(request):
    ...
    # wake goes into beautifier + notify
    send_message("Wake", f"wake {text}", priority=5, extras={"source": "wake"})
    return web.json_response({"status": "ok"})

# --- NEW: chat endpoint ---
async def _internal_chat(request):
    try:
        data = await request.json()
    except Exception:
        return web.json_response({"error": "invalid json"}, status=400)

    query = str(data.get("text") or "").strip()
    if not query:
        return web.json_response({"error": "no query"}, status=400)

    try:
        from llm_client import persona_riff
        reply = persona_riff(query, persona=ACTIVE_PERSONA)
    except Exception as e:
        reply = f"(chat error: {e})"

    # broadcast as if it was a normal outgoing message
    send_message("Chat", reply, priority=5, extras={"source": "jarvis_out"}, decorate=False)
...
def _start_internal_server():
    ...
    app.router.add_post("/chat", _internal_chat)  # <— ADDITIVE
    runner = web.AppRunner(app)

    async def _run():
        await runner.setup()
        site = web.TCPSite(runner, "0.0.0.0", 2599)
        await site.start()
        print("[bot] internal aiohttp server running on :2599")
