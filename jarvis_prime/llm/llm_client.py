...
# ============================
class ModelUtils:
    """Model-specific detection and configuration"""
    
    @staticmethod
    def is_phi3_family() -> bool:
        """Detect if loaded model is from Phi-3 family"""
        combined = " ".join([_MODEL_ARCH, _CHAT_TEMPLATE]).lower()
        return ("phi3" in combined) or ("<|user|>" in combined and "<|assistant|>" in combined and "<|end|>" in combined)
    
    @staticmethod
    def get_stop_tokens() -> List[str]:
        """Get appropriate stop tokens for current model"""
        if ModelUtils.is_phi3_family():
            return ["<|end|>", "<|endoftext|>"]
        return ["</s>", "[/INST]"]
    
    @staticmethod
    def should_use_grammar_auto() -> bool:
        """Determine if grammar should be used automatically"""
        if ModelUtils.is_phi3_family():
            return False
        
        template_lower = (_CHAT_TEMPLATE or "").lower()
        return "INST" in _CHAT_TEMPLATE or "llama" in template_lower
    
    @staticmethod
    def get_model_name_from_url(model_url: str) -> str:
        ...
        return filename or "llama3" Exception as e:
                _log(f"Lock release error: {e}")
...
class ModelManager:
    ...
    def coerce_model_path(model_url: str, model_path: str) -> str:
        """Convert model URL/path to concrete local path"""
    ...
    def verify_sha256(file_path: str, expected_sha256: str) -> bool:
        ...
        try:
            ...
            if actual_hash.lower() != expected_sha256.lower():
                _log(f"SHA256 mismatch: got={actual_hash} expected={expected_sha256}")
                return False
            return True
    ...
    @staticmethod
    def ensure_local_model(model_url: str, model_path: str, token: Optional[str], 
                          expected_sha256: str) -> Optional[str]:
        """Ensure model is available locally"""
        local_path = ModelManager.coerce_model_path(model_url, model_path)
        ...
        if not Path(local_path).exists():
            ...
            if not HTTPClient.download_file(model_url, local_path, token):
                return None
        
        # Verify hash if provided
        if not ModelManager.verify_sha256(local_path, expected_sha256):
            return None
    ...
    def update_model_metadata():
        ...
        try:
            ...
            if isinstance(metadata, dict):
                _MODEL_ARCH = str(metadata.get("general.architecture", ""))
                _CHAT_TEMPLATE = str(metadata.get("tokenizer.chat_template", ""))
