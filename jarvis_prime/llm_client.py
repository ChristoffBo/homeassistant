...
def _strip_meta_lines(text: str) -> str:
    """Drop any instruction/prompt-y lines the model might echo."""
    BAD = (
        "persona", "rules", "rule:", "instruction", "instruct", "guideline",
        "you are jarvis", "you are  jarvis", "jarvis prime", "system prompt",
        "style hint", "speak as", "lines:", "produce at most", "respond with at most",
        "do not", "no labels", "no meta", "ops/lang"
    )
    out = []
    for ln in (text or "").splitlines():
        t = ln.strip()
        if not t:
            continue
        low = t.lower()
        # Kill lines that end with ":" or look like section headers
        if low.endswith(":"):
            continue
        if any(b in low for b in BAD):
            continue
        # Drop all-caps prefix tokens like OPS/, SYS/, NOTE/
...
def _finalize(text: str, imgs: str) -> str:
    ...
    out = _strip_meta_lines(out)          # strip persona/rules/instruction echoes
...
def _cleanup_quip_block(text: str, max_lines: int) -> List[str]:
    ...
    # remove bullets/numbers and quote markers
    s = re.sub(r'^\s*[-•>\d\)\.]+\s*', '', s, flags=re.M)
    # split reliably into candidate lines
    lines = [ln.strip(" \t\r\n\"'") for ln in s.splitlines() if ln.strip()]
    out: List[str] = []
    seen: set[str] = set()
    for ln in lines:
        # nuke any residual labels (early)
        if ":" in ln[:20]:
            # drop lines that look like "OPS:" / "NOTE:" / "RULES:"
            if re.match(r"^[A-Za-z\/ _-]{2,20}:\s*", ln):
                continue
        # cap length ~30 words
        words = ln.split()
        if len(words) > 30:
        ...
        if len(out) >= max_lines:
            break
    # final safety: if nothing clean, return empty (no garbage)
    return out
...
def rewrite(text: str, mood: str = "serious", timeout: int = 8, cpu_limit: int = 70,
    ...
    common_stops = ["[SYSTEM]", "[INPUT]", "[OUTPUT]", "Persona:", "Rules:", "OPS/LANG", "YOU ARE", "System prompt"]
...
def persona_riff(persona: str, context: str, max_lines: int = 3, timeout: int = 8,
    ...
    """
    Generate 1–N short persona lines about `context`. Clean, no meta/rules echoed.
    """
    ...
    # Minimal, strict instruction. No meta words that models tend to echo.
    instruction = (
        f"Speak as {persona}. Give up to {max_lines} short lines. "
        "Each line under 30 words. No labels or headings. No rules. Output only the lines."
    )

    common_stops = ["Lines:", "Rules:", "Persona:", "[SYSTEM]", "[INPUT]", "[OUTPUT]", "YOU ARE", "System prompt", "OPS/LANG"]
...
# Provide a friendly alias some modules might expect
