...
def _cfg_from(merged: dict) -> Dict[str, Any]:
    ...
    try:
        ...
        cfg["ha_url"] = str(merged.get("llm_enviroguard_ha_base_url") or merged.get("ha_base_url") or merged.get("ha_url") or cfg["ha_url"]).strip()
        cfg["ha_token"] = str(merged.get("llm_enviroguard_ha_token") or merged.get("ha_token") or cfg["ha_token"]).strip()
        cfg["ha_temperature_entity"] = str(merged.get("llm_enviroguard_ha_temp_entity") or merged.get("ha_indoor_temp_entity") or merged.get("ha_temperature_entity") or cfg["ha_temperature_entity"]).strip()
...
def _next_profile_with_hysteresis(temp_c: float, last_profile: str, cfg: Dict[str, Any]) -> str:
    ...
    if lp == "off" and temp_c <= off_c - hyst: return band_of(temp_c)
    if lp == "hot" and temp_c <= hot_c - hyst: return band_of(temp_c)
    if lp == "boost" and temp_c >= boost_c + hyst: return band_of(temp_c)
    if lp == "cold" and temp_c >= cold_c + hyst: return band_of(temp_c)
    return target
...
def get_boot_status_line(merged: dict) -> str:
    ...
    if not cfg.get("enabled"):
        active_line = f"🌡️ EnviroGuard — OFF (mode={mode.upper()})"
    else:
        active_line = "🌡️ EnviroGuard — ACTIVE"
    env_line = f"ENV MODE={mode.upper()}, PROFILE={prof.upper()}, SRC={src}"
    if t is not None:
        env_line += f", {t:.1f}°C"
    return f"{active_line} | {env_line}"
    ...
    if w in profiles:
        ...
        if callable(send_message):
            try:
                ...
                send_message("EnviroGuard", f"{prefix} → profile **{w.upper()}** (CPU={merged.get('llm_max_cpu_percent')}%, ctx={merged.get('llm_ctx_tokens')}, to={merged.get('llm_timeout_seconds')}s)", priority=4, decorate=False)
            except Exception: pass
        return True
    return False
    ...
    while True:
        try:
            ...
            if _state.get("mode","auto") == "auto" and temp_c is not None:
                ...
                else: nextp = _next_profile_with_hysteresis(temp_c, last, cfg)
                if nextp != last:
                    _apply_profile(nextp, merged, cfg)
                    if callable(send_message):
                        try:
                            send_message("EnviroGuard", f"{source or 'temp'} {temp_c:.1f}°C → profile **{nextp.upper()}** (CPU={merged.get('llm_max_cpu_percent')}%, ctx={merged.get('llm_ctx_tokens')}, to={merged.get('llm_timeout_seconds')}s)", priority=4, decorate=False)
                        except Exception: pass
        except Exception as e:
