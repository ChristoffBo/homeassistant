...
try:
    ...
    DNS_AVAILABLE = False
    logger.warning("dnspython not available - some features may be limited")
...
class DNSSECValidator:
    """DNSSEC validation using dnspython"""
    ...
    async def validate(self, domain: str, response: bytes) -> bool:
        """Validate DNSSEC for domain and response"""
        if not self.enabled:
            return True
        
        if not DNS_AVAILABLE:
            # If dnspython not available, skip validation
            return True
        
        # Check cache first
        cache_key = f"{domain}:{hashlib.md5(response).hexdigest()}"
        if cache_key in self._cache:
            return self._cache[cache_key]
        
        try:
            # Parse response to check for DNSSEC records
            msg = dns.message.from_wire(response)
            
            # Check if response has DNSSEC records (RRSIG, DNSKEY, DS, NSEC, NSEC3)
            has_dnssec = False
            for section in [msg.answer, msg.authority, msg.additional]:
                for rrset in section:
                    if rrset.rdtype in [dns.rdatatype.RRSIG, dns.rdatatype.DNSKEY, 
                                       dns.rdatatype.DS, dns.rdatatype.NSEC, 
                                       dns.rdatatype.NSEC3]:
                        has_dnssec = True
                        break
                if has_dnssec:
            ...
            # If no DNSSEC records, assume OK (not all domains have DNSSEC)
            if not has_dnssec:
                self._cache[cache_key] = True
                return True
            
            # Use dig as fallback for actual validation
            try:
                proc = await asyncio.create_subprocess_exec(
                    'dig', '+dnssec', '+noall', '+answer', domain,
                    stdout=asyncio.subprocess.PIPE,
                    stderr=asyncio.subprocess.PIPE
                )
                stdout, stderr = await asyncio.wait_for(proc.communicate(), timeout=2.0)
                
                # If dig returns SERVFAIL, DNSSEC validation failed
                if b'SERVFAIL' in stdout or b'SERVFAIL' in stderr:
                    self._cache[cache_key] = False
                    return False
                
                # If we got records back, validation passed
                if proc.returncode == 0:
                    self._cache[cache_key] = True
                    return True
            
            except asyncio.TimeoutError:
                logger.warning(f"DNSSEC validation timeout for {domain}")
            except Exception as e:
                logger.warning(f"DNSSEC dig validation error for {domain}: {e}")
            
            # Default to allowing if validation is inconclusive
            self._cache[cache_key] = True
            return True
...
class VeilDNS:
    ...
    async def load_blocklists(self):
        ...
        for url in blocklist_urls:
            ...
            # Download and save locally if enabled
            if self.config['blocking']['local_blocklist_storage']:
                if not filepath.exists():
                    try:
                        logger.info(f"[blocklist] Downloading: {url}")
                        async with self.session.get(url) as resp:
                            if resp.status == 200:
                                ...
                                with open(filepath, 'w') as f:
                                    f.write(content)
                                logger.info(f"[blocklist] Saved to {filepath}")
                    except Exception as e:
                        logger.error(f"[blocklist] Download failed {url}: {e}")
                        continue
            ...
            if filepath.exists():
                try:
                    ...
                    logger.info(f"[blocklist] Loaded from {filepath}")
                except Exception as e:
                    logger.error(f"[blocklist] Load failed {filepath}: {e}")
    ...
    async def handle_dns_query(self, query: bytes, addr: tuple) -> bytes:
        ...
        if safe_domain:
            ...
            # Rebuild query with safe domain if dnspython available
            if DNS_AVAILABLE:
                try:
                    original_msg = dns.message.from_wire(query)
                    qtype = original_msg.question[0].rdtype
                    qclass = original_msg.question[0].rdclass
                    
                    # Create new query for safe domain
                    safe_query = dns.message.make_query(safe_domain, qtype, qclass)
                    safe_query.id = original_msg.id
                    query = safe_query.to_wire()
                    domain = safe_domain
                except Exception as e:
                    logger.warning(f"SafeSearch DNS rebuild failed: {e}, using domain substitution")
                    domain = safe_domain
            else:
                # Fallback: just update the domain variable
                domain = safe_domain
    ...
    async def run_doq_server(self):
        ...
        # DoQ implementation using UDP socket with QUIC protocol
        # Note: Full production DoQ requires aioquic library
        # This is a basic implementation for compatibility
        logger.info("DoQ server running on UDP port {self.config['doq_port']}")
        ...
        try:
            while True:
                try:
                    # Receive QUIC packet
                    data, addr = await loop.sock_recvfrom(self.doq_socket, 4096)
                    
                    # Basic QUIC packet detection (starts with header flags)
                    if len(data) < 20:
                        continue
                    
                    # Try to extract DNS query from QUIC payload
                    # QUIC has complex framing, so this is simplified
                    # In production, use aioquic library
                    try:
                        # Skip QUIC headers (simplified - typically 17-40 bytes)
                        # Look for DNS message (starts with transaction ID)
                        for offset in range(0, min(60, len(data) - 12)):
                            potential_query = data[offset:]
                            if len(potential_query) >= 12:
                                try:
                                    # Try to process as DNS
                                    response = await self.handle_dns_query(potential_query, addr)
                                    if response:
                                        # Send response back (wrapped in minimal QUIC framing)
                                        # In production, proper QUIC framing required
                                        self.doq_socket.sendto(response, addr)
                                        break
                                except:
                                    continue
                    except:
                    ...
                    logger.debug(f"DoQ error: {e}")
                    await asyncio.sleep(0.1)
        
        except asyncio.CancelledError:
            pass
        finally:
            logger.info("DoQ server stopped")
