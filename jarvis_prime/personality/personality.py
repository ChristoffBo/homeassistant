...
# === ADDITIVE: Neutral LLM rewrite helper ====================================
# Drop this whole block at the VERY END of personality.py (no edits above)

import re as _rw_re

def _rw_compact(s: str) -> str:
    """Normalize whitespace/newlines; keep simple paragraph breaks."""
    if not s:
        return ""
    # Collapse 3+ newlines -> 2; trim trailing spaces; squeeze inner spaces.
    s = _rw_re.sub(r'[ \t]+', ' ', s)
    s = _rw_re.sub(r'\n{3,}', '\n\n', s)
    return s.strip()

def _rw_wrap_lines(s: str, max_lines: int = 3, max_len: int = 140) -> str:
    """
    Greedy word-wrap into up to `max_lines` lines, each ≤ `max_len` chars.
    Does not split words; last line may be shorter.
    """
    words = s.split()
    if not words:
        return ""
    ...
    for w in words:
        ...
        if len(nxt) <= max_len:
            ...
            if len(lines) >= max_lines - 1:
                # Put the rest in the final line (truncate softly if needed)
                rest = " ".join(words[words.index(w):]).strip()
                if len(rest) > max_len:
...
def _rw_soft_trim(s: str, max_chars: int = 800) -> str:
    """Soft cap total characters without cutting mid-word too harshly."""
    if len(s) <= max_chars:
        return s
    sub = s[:max_chars]
    cut = max(sub.rfind("\n"), sub.rfind(" "))
    if cut < int(max_chars * 0.6):
        cut = max_chars
    return sub[:cut].rstrip()

def llm_rewrite(persona_name: str, *, context: str, max_chars: int = 800) -> str:
    """
    Neutral rewrite/summarize helper used by beautifier.
    - Persona is IGNORED for tone (kept neutral/ops-friendly).
    - Targets concise clarity; no jokes/puns; no persona flavor.
    - Bounded for CPU via short timeout and modest token/context.
    Returns a string (possibly multi-line), or the original `context` if LLM is unavailable.
    """
    raw = _rw_compact(context or "")
    if not raw:
        return ""

    # Build a terse, neutral instruction. We intentionally avoid persona styling here.
    sys_prompt = (
        "You are a concise rewrite assistant. Improve clarity and brevity without changing facts. "
        "Keep plain, neutral tone. No jokes, no puns, no emojis, no formatting beyond simple text. "
        "Prefer short, readable sentences."
    )
    user_prompt = (
        "Rewrite the following text neutrally and clearly. Keep important numbers, names, and statuses. "
        "If lists are implied, compress to compact sentences.\n\n"
        f"Text:\n{raw}"
    )

    # Lazy import; if unavailable, fall back to the original text.
    try:
        import llm_client as _rw_llm
    except Exception:
        # No LLM available—return compacted original, wrapped.
        compact_only = _rw_wrap_lines(_rw_soft_trim(raw, max_chars=max_chars), max_lines=3, max_len=140)
        return compact_only or raw

    # Ask the local LLM (Phi-3 or equivalent via llm_client) with tight budget.
    try:
        out = _rw_llm.rewrite(
            text=f"[SYSTEM]\n{sys_prompt}\n\n[INPUT]\n{user_prompt}\n\n[OUTPUT]\n",
            mood="neutral",                 # enforce neutral tone
            timeout=6,                      # short, CPU-friendly
            cpu_limit=60,                   # modest CPU share
            ctx_tokens=2048,                # conservative context
            model_url="",                   # resolved by options.json in llm_client
            model_path="",                  # resolved by options.json in llm_client
            model_sha256="",
            allow_profanity=False,
            max_lines=0,                    # we handle wrapping below
            max_chars=0                     # we handle soft trim below
        ) or ""
    except Exception:
        out = ""

    final = _rw_compact(out) if out else raw
    final = _rw_soft_trim(final, max_chars=max_chars)
    final = _rw_wrap_lines(final, max_lines=3, max_len=140)
    return final or raw
```0
