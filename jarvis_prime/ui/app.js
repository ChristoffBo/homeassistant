 ...
 * Additive only. Works if Aegis tabs/elements exist in index.html.
 * ======================================================================= */
(function (){
  const $  = (s) => document.querySelector(s);
  const $$ = (s) => document.querySelectorAll(s);

  function aegToast(msg){ try{ if(window.toast) return window.toast(msg); }catch{} console.log('[Aegis]',msg); }

  function base(){ return (window.JARVIS_API_BASE||location.origin+'/').replace(/\/?$/,'/'); }
  const AEGIS_API = (p)=> new URL('api/aegisops/'+p, base()).toString();

  async function jfetch(url,opt){ const r=await fetch(url,opt); const ct=r.headers.get('content-type')||''; return ct.includes('json')?r.json():r.text(); }

  async function refreshHosts(){
    const txt = await jfetch(AEGIS_API('inventory'));
    $('#ag-hosts').textContent = txt;
  }
  $('#ag-host-refresh')?.addEventListener('click', refreshHosts);

  $('#ag-pb-refresh')?.addEventListener('click', async()=>{
    const list = await jfetch(AEGIS_API('playbooks'));
    console.log('Playbooks', list);
  });

  $('#ag-add')?.addEventListener('click', async()=>{
    const id=$('#ag-sch-id').value.trim();
    if(!id) return aegToast('Need ID');
    const body={id,playbook:$('#ag-sch-playbook-select').value,every:$('#ag-sch-every').value};
    await fetch(AEGIS_API('schedules'),{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify(body)});
    aegToast('Schedule saved');
  });

  $('#ag-refresh')?.addEventListener('click', async()=>{
    const runs = await jfetch(AEGIS_API('runs'));
    console.log('Runs',runs);
  });

  $('#pe-save')?.addEventListener('click', async()=>{
    const name=$('#pe-name').value.trim();
    const body=$('#pe-body').value;
    if(!name) return aegToast('Name needed');
    await fetch(AEGIS_API('playbook'),{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({name,content:body})});
