// Jarvis v3.3 — ingress-safe + auto-refresh + "Archived"
(function(){
  ...
  const els = {
    ...
    quicks: document.querySelectorAll('.quick-actions .chip')
  };

  // Ingress-aware API root: remove trailing '/ui...' from pathname
  function apiRoot(){
    const {origin, pathname} = location;
    const cut = pathname.split('/ui')[0];
  ...
  const API = {
    async list(q, limit=50, offset=0, archivedOnly=null){
      const url = new URL(u('api/messages'));
      if(q) url.searchParams.set('q', q);
      url.searchParams.set('limit', limit);
      url.searchParams.set('offset', offset);
      if(archivedOnly!==null) url.searchParams.set('saved', archivedOnly ? '1':'0'); // backend uses saved
    ...
    async setArchived(id,val){ const r=await fetch(u(`api/messages/${id}/save`),{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({saved:val})}); if(!r.ok) throw new Error('archive'); return r.json(); },
  ...
  let state = { items:[], active:null, archivedOnly:false };
  ...
  function renderList(items){
    ...
    for(const it of items){
      ...
      div.innerHTML = `<div><div class="title">${esc(it.title||'(no title)')}</div><div class="meta">${fmt(it.created_at)} • <span class="source">${esc(it.source||'?')}</span>${it.saved?' • Archived':''}</div></div><div class="meta">#${it.id}</div>`;
  ...
  async function renderPreview(it){
    ...
    root.querySelector('#a-arch').addEventListener('click', async()=>{ const want=!it.saved; await API.setArchived(it.id,want); load(it.id); });
  }
  ...
  async function load(selectId=null){ const q=els.q.value.trim(); const lim=parseInt(els.limit.value,10)||50; const items=await API.list(q,lim,0,state.archivedOnly); state.items=items; renderList(items); if(selectId && items.find(i=>i.id===selectId)){ select(selectId); } else if(items[0]) select(items[0].id); else els.preview.innerHTML='<div class="empty"><h2>Inbox</h2><p>No messages yet.</p></div>'; }
  ...
  function startLive(){
    ...
    setInterval(()=> load(state.active), 8000);
  }
  ...
  els.archivedBtn.addEventListener('click', ()=>{
    state.archivedOnly=!state.archivedOnly; els.archivedBtn.textContent='Archived only: '+(state.archivedOnly?'ON':'OFF'); load(state.active);
  });

  els.saveRetention.addEventListener('click', async()=>{ const d=parseInt(els.retention.value,10)||30; await API.setRetention(d); footer('Retention saved'); });
  els.purge.addEventListener('click', async()=>{
    ...
    const d=parseInt(v,10)||30; if(!confirm(`Purge messages older than ${d} days?`)) return; await API.purge(d); footer('Purge started'); load();
  });
  $('#btn-delall').addEventListener('click', async()=>{ if(!confirm('Delete ALL messages?')) return; await API.deleteAll(els.keepFav.checked); footer('All deleted'); load(); });

  function footer(msg){ els.footer.textContent = msg; setTimeout(()=> els.footer.textContent='', 2000); }
  ...
  els.wakeSend.addEventListener('click', async()=>{
    ...
    try{
      ...
      const temp = { id: 'ui-'+now, title:'Wake', message:t, source:'ui', created_at: now, saved:false };
      state.items.unshift(temp); renderList(state.items); select(temp.id);
      footer('Wake sent');
      els.wakeText.value='';
      setTimeout(()=> load(temp.id), 1500);
    }catch{ footer('Wake failed'); }
  });
  els.quicks.forEach(btn => btn.addEventListener('click', ()=>{ els.wakeText.value = btn.dataset.wake; els.wakeSend.click(); }));
