...
if jq -e '.nas_mounts | length >= 1' "$CONFIG_PATH" >/dev/null 2>&1; then
  while IFS= read -r row; do
    proto=$(echo "$row" | jq -r '.proto // "cifs"')
    server=$(echo "$row" | jq -r '.server // empty')
    share=$(echo "$row" | jq -r '.share // empty')
    mountp=$(echo "$row" | jq -r '.mount // "/mnt/nas"')
    user=$(echo "$row" | jq -r '.username // empty')
    pass=$(echo "$row" | jq -r '.password // empty')
    mkdir -p "$mountp"
    if [ "$proto" = "cifs" ]; then
      opts="rw,iocharset=utf8,file_mode=0777,dir_mode=0777,vers=3.0"
      [ -n "$user" ] && opts="$opts,username=$user"
      [ -n "$pass" ] && opts="$opts,password=$pass"
      mount -t cifs "//$server/$share" "$mountp" -o "$opts" || echo "WARN: CIFS mount failed for //$server/$share -> $mountp"
    else
      mount -t nfs "$server:$share" "$mountp" || echo "WARN: NFS mount failed for $server:$share -> $mountp"
    fi
  done < <(jq -c '.nas_mounts[]' "$CONFIG_PATH")
fi
