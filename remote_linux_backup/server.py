...
def api_mounts_listdir():
    ...
    if m.get("type")=="smb":
        rel2 = rel.lstrip("/")
        items = smb_ls(m.get("host",""), m.get("share",""), m.get("username",""), m.get("password",""), rel2 if rel2 else "/")
        return jsonify({"ok":True,"base":"//%s/%s"%(m.get("host",""),m.get("share","")), "path":"/"+rel2, "items":items, "fallback":True})
...
def api_mounts_mkdir():
    ...
    if m.get("type")=="smb":
        rel2 = (rel.strip('/') + '/' if rel.strip('/') else '') + folder
        auth = f"-U {shlex.quote(m.get('username','')+'%'+m.get('password',''))}" if m.get("username") else "-N"
        cmd = f"smbclient //{shlex.quote(m.get('host',''))}/{shlex.quote(m.get('share',''))} {auth} -c " + shlex.quote(f"mkdir {rel2}")
        rc,out,err = run_cmd(cmd + " 2>&1 || true")
        ok = (rc==0) or ('created directory' in (out or '').lower())
        return jsonify({"ok": ok, "path":"/"+rel2})
