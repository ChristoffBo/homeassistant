...
class JobWorker(threading.Thread):
    ...
    def submit(self, job):
        with self.lock:
            self.queue.append(job); self.cv.notify_all()
    ...
    def run(self):
        while True:
            with self.lock:
                ...
                job["status"]="running"; job["progress"]=0; job["log"]=[]; job["started"]=int(time.time())
            try:
                ...
                hist=read_json(PATHS["history"],[]); hist.append({k:job.get(k) for k in ("id","kind","label","status","started","ended","dest","mode")}); write_json(PATHS["history"],hist)
...
def api_connections(): return jsonify({"connections": read_json(PATHS["connections"], [])})
...
def api_connections_save():
    ...
    data.append({"name":b.get("name","").strip(), "host":b.get("host","").strip(), "port":int(b.get("port") or 22),
                 "username":b.get("username","").strip(), "password":b.get("password","")})
    write_json(PATHS["connections"], data); return jsonify({"ok": True})

@app.post("/api/ssh/test")
def api_ssh_test():
    b=request.json or {}
    host=b.get("host",""); port=int(b.get("port") or 22); user=b.get("username",""); pw=b.get("password","")
...
def api_ssh_listdir():
    ...
    if paramiko:
        try:
            ...
            items=[{"name":e.filename,"dir":bool(e.st_mode & 0o040000),"size":e.st_size} for e in sftp.listdir_attr(path)]
            sftp.close(); t.close()
...
def api_local_listdir():
    ...
    try:
        ...
        for e in os.scandir(path):
            items.append({"name":e.name, "dir": e.is_dir(), "size": (0 if e.is_dir() else e.stat().st_size)})
        return jsonify({"ok": True, "items": items})
    except Exception as e:
...
def api_mounts_save():
    ...
    data=read_json(PATHS["mounts"], []); data=[x for x in data if x.get("name")!=name]
...
def api_mounts_mount():
    name=(request.json or {}).get("name",""); data=read_json(PATHS["mounts"], [])
    ...
    if m["type"]=="smb":
        unc=f"//{m['host']}/{m['share']}"; opts=m.get("options","")
        ...
        opts=m.get("options",""); cmd=f"mount -t nfs {shlex.quote(m['host']+':'+export)} {shlex.quote(mp)}" + (f" -o {shlex.quote(opts)}" if opts else "")
    rc,out,err=run_cmd(cmd + " 2>&1 || true")
    ok=os.path.ismount(mp)
    if not ok and m["type"]=="smb":
        m["last_error"]="kernel mount failed; userspace smbclient fallback will be used"; write_json(PATHS["mounts"], data)
        return jsonify({"ok": False, "error": m["last_error"]})
    if not ok:
        m["last_error"]=out or err; write_json(PATHS["mounts"], data); return jsonify({"ok": False, "error": out or err})
    m["last_error"]=""; write_json(PATHS["mounts"], data); return jsonify({"ok": True})

@app.post("/api/mounts/unmount")
def api_mounts_unmount():
    name=(request.json or {}).get("name",""); rc,out,err=run_cmd(f"umount {shlex.quote(mountpoint(name))} 2>&1 || true"); return jsonify({"ok": True})
...
def api_mounts_listdir():
    b=request.json or {}; name=b.get("name",""); rel=b.get("path","/"); mp=mountpoint(name)
    if not name or not os.path.exists(mp): return jsonify({"ok": False, "error": "mount not found"}), 400
...
def api_estimate():
    ...
    def _du(cmd): rc,out,err=run_cmd(cmd+" 2>&1 || true"); 
    # local
    if mode=="local":
        rc,out,err=run_cmd(f"du -s -B1 {shlex.quote(path)} 2>/dev/null || du -sb {shlex.quote(path)} 2>/dev/null || du -sk {shlex.quote(path)}")
        try:
            size=int(out.split()[0]); 
            if out.strip().endswith("\t"+path) and ' -sk ' in out: size*=1024
        except: size=0
        return jsonify({"ok": True, "bytes": size})
    elif mode=="mount":
        name=b.get("name",""); mp=mountpoint(name); real=os.path.normpath(os.path.join(mp, path.lstrip("/")))
        rc,out,err=run_cmd(f"du -s -B1 {shlex.quote(real)} 2>/dev/null || du -sb {shlex.quote(real)} 2>/dev/null || du -sk {shlex.quote(real)} 2>/dev/null")
        try:
            size=int(out.split()[0]); 
            if out.strip().endswith("\t"+real) and ' -sk ' in out: size*=1024
        except: size=0
        return jsonify({"ok": True, "bytes": size})
    elif mode=="ssh":
        host=b.get("host"); user=b.get("username"); pw=b.get("password")
        keep="-o ServerAliveInterval=30 -o ServerAliveCountMax=6 -p 22"
        remote=f"du -s -B1 {shlex.quote(path)} 2>/dev/null || du -sb {shlex.quote(path)} 2>/dev/null || du -sk {shlex.quote(path)}"
        cmd=f"sshpass -p {shlex.quote(pw)} ssh -o StrictHostKeyChecking=no {keep} {shlex.quote(user)}@{shlex.quote(host)} {remote}"
        rc,out,err=run_cmd(cmd+" 2>&1 || true")
        try:
            size=int(out.split()[0]); 
            if " -sk " in remote: size*=1024
        except: size=0
...
def api_upload():
    ...
    name=secure_filename(f.filename); dest=os.path.join(UPLOAD_DIR, name); f.save(dest)
...
def api_health():
    ...
    try:
        parts=out.split(); free_mb=int(parts[3])//1024
    except: free_mb=None
    total_sz=0; count=0
    for root,_,files in os.walk(BACKUP_DIR):
        for f in files:
    ...
    next_run=0; sch=read_json(PATHS["schedules"], [])
    if sch: next_run=min((s.get("next_run",0) or 0) for s in sch if s.get("next_run"))
    try:
        up_secs=float(open("/proc/uptime").read().split()[0])
    except: up_secs=None
    return jsonify({
        "backup_dir": BACKUP_DIR, "free_mb": free_mb,
        "backup_files": count, "backup_bytes": total_sz,
...
def api_backup_start():
    ...
    dest_type=b.get("dest_type","local"); dest_mount=b.get("dest_mount_name","")
    bwkb=int(b.get("bwlimit_kbps") or 0); dry=bool(b.get("dry_run", False))
    ...
    out_dir=os.path.join(dest_root, f"{label}-{int(time.time())}"); os.makedirs(out_dir, exist_ok=True)
    ...
    if mode=="rsync":
        ...
        name=b.get("mount_name"); src=b.get("source_path","/"); mp=mountpoint(name)
        if os.path.ismount(mp):
            ...
            mounts=read_json(PATHS["mounts"], []); m=next((x for x in mounts if x.get("name")==name), None)
...
def api_restore_start():
    b=request.json or {}
    from_id=b.get("from_id",""); to_mode=b.get("to_mode","local"); to_path=b.get("to_path","/")
...
def api_notify_set(): write_json(PATHS["notify"], request.json or {}); return jsonify({"ok": True})

@app.post("/api/notify/test")
def api_notify_test():
    cfg=read_json(PATHS["notify"], {})
    if not cfg.get("url") or not cfg.get("token"): return jsonify({"ok": False, "error":"missing url/token"})
    url=cfg["url"].rstrip("/") + "/message"; pr=int(cfg.get("priority",5))
    ...
    rc,out,err=run_cmd(cmd+" 2>&1 || true"); return jsonify({"ok": rc==0, "out": out or err})

# ----------------- Schedules (simple) -----------------
def _tick_schedules():
    while True:
        sch=read_json(PATHS["schedules"], []); now=time.time(); changed=False
        for s in sch:
            if not s.get("enabled"): continue
            nxt=s.get("next_run",0) or 0
            if nxt and nxt>now: continue
            if not nxt: pass
            body=s.get("template",{}); body["label"]=s.get("name","job")
            try:
                with app.test_request_context():
                    request.json=body; api_backup_start()
            except Exception: pass
            # calc next
            freq=s.get("frequency","daily"); tm=s.get("time","02:00"); day=int(s.get("day",0))
            ...
            if freq=="daily":
                ...
                m=dt.month+1; y=dt.year+(1 if m>12 else 0); m=1 if m>12 else m; d=max(1,min(28, day or 1))
                dt=datetime.datetime(y,m,d,hour=hh,minute=mm)
            s["next_run"]=int(dt.timestamp()); changed=True
        if changed: write_json(PATHS["schedules"], sch)
        time.sleep(30)
threading.Thread(target=_tick_schedules, daemon=True).start()

@app.get("/api/schedules")
def api_schedules_get(): return jsonify({"schedules": read_json(PATHS["schedules"], [])})

@app.post("/api/schedules/save")
def api_schedules_save():
    b=request.json or {}; sch=read_json(PATHS["schedules"], [])
    sch=[x for x in sch if x.get("name")!=b.get("name")]
    b.setdefault("enabled",True); b.setdefault("frequency","daily"); b.setdefault("time","02:00"); b.setdefault("day",0); b.setdefault("template",{}); b.setdefault("next_run",0)
    sch.append(b); write_json(PATHS["schedules"], sch); return jsonify({"ok": True})
