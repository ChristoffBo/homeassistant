...
<td class="row"><button class="mini use_dst" data-name="${m.name}">Use as destination</button><button class="mini use_src" data-name="${m.name}">Use as source</button><button class="mini mnt" data-name="${m.name}">${m.mounted?'Unmount':'Mount'}</button><button class="mini danger del" data-name="${m.name}">Delete</button></td>`; 
  ...
  tb.querySelectorAll('.del').forEach(b=>b.onclick=async()=>{ if(!confirm('Delete this mount?')) return; const r=await j('/api/mounts/delete',{method:'POST',body:JSON.stringify({name:b.dataset.name})}); logInto('#log_mounts','delete: '+JSON.stringify(r)); refreshMounts(); });
}
