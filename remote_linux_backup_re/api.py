...
# App-owned config (UI saves here)
...
def save_app_config(data: dict):
    ...
    for k, v in data.items():
        if isinstance(v, (str, int, float, bool)) or v is None or isinstance(v, (list, dict)):
            # Normalize comma/newline separated strings for known list fields
...
def dd_backup(user, host, password, disk, out_path, port=22, verify=False, bwlimit_kbps=None):
    ...
    if rc==0:
        ...
        with open(sha_path, "w") as sf: sf.write(f"{h.hexdigest()}  {os.path.basename(out_path)}\n")
        if verify:
            hv = hashlib.sha256()
            with open(out_path, "rb") as f:
                for chunk in iter(lambda: f.read(1024*1024), b""): hv.update(chunk)
            if hv.hexdigest()!=h.hexdigest(): out += "\n[VERIFY] SHA256 mismatch!"; rc = 2
            else: out += "\n[VERIFY] SHA256 OK"
    return rc, out
...
def estimate_backup():
    ...
    if method=="dd":
        ...
        rc,out = ssh(user,host,pwd,f"blockdev --getsize64 {shlex.quote(disk)} 2>/dev/null || cat /sys/block/$(basename {shlex.quote(disk)})/size 2>/du/dev/null", port=port)
