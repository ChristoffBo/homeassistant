...
# (Include all your existing fetch_latest_dockerhub_tag, fetch_latest_linuxserver_tag, fetch_latest_ghcr_tag, get_latest_docker_tag functions here without change)
...
update_addon_if_needed() {
  ...
  if [ ! -f "$config_file" ] && [ ! -f "$build_file" ] && [ ! -f "$updater_file" ]; then
    log "$COLOR_YELLOW" "⚠️ Add-on '$(basename "$addon_path")' has no config.json, build.json, or updater.json, skipping."
    return 1
  fi
  ...
  log "$COLOR_BLUE" "🕒 Last updated: ${upstream_version:-'never'}"
  ...
  if [ "$latest_version" != "$current_version" ] && [ "$latest_version" != "latest" ]; then
    ...
    local NEW_ENTRY="\
v$latest_version ($(date -u +'%d-%m-%Y %H:%M'))
...
perform_update_check() {
  ...
  log "$COLOR_BLUE" "🧮 Total add-ons found: $addon_count"
  log "$COLOR_BLUE" "🔄 Add-ons updated: $updated_count"

  if [ "$(git status --porcelain)" ]; then
    git add .
    if git commit -m "⬆️ Update add-ons ($updated_count updated)" >> "$LOG_FILE" 2>&1; then
      log "$COLOR_GREEN" "✅ Git commit successful."
    else
      log "$COLOR_YELLOW" "⚠️ Git commit failed or nothing to commit."
    fi
...
log "$COLOR_BLUE" "📅 Scheduled daily at $CHECK_TIME (UTC)"
log "$COLOR_GREEN" "🏃 Running initial update check on startup..."
perform_update_check

# Setup cron to trigger daily at $CHECK_TIME
echo "$CHECK_TIME * * * /run.sh --cron-trigger" > /etc/crontabs/root
crond -f -L /data/crond.log
