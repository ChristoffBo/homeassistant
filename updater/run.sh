...
# Function to sanitize version strings
sanitize_version() {
...
get_latest_docker_tag() {
  ...
  for ((i=1; i<=retries; i++)); do
    ...
    # Sanitize and validate the version
...
update_addon_if_needed() {
    ...
    if [[ -f "$config_file" ]]; then
        ...
        current_version=$(sanitize_version "$(jq -r '.version | select(.!=null)' "$config_file" 2>/dev/null || echo "latest")")
    fi
    ...
    upstream_version=$(sanitize_version "$(jq -r '.upstream_version | select(.!=null)' "$updater_file" 2>/dev/null || echo "latest")")
