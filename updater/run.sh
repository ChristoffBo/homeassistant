...
get_latest_tag() { local image="$1" local arch=$(uname -m) arch=${arch//x86_64/amd64} arch=${arch//aarch64/arm64} image="${image//{arch}/$arch}" local image_name="${image%%:*}" local cache_file="/tmp/tags_$(echo "$image_name" | tr '/' '_').txt"
...
if [[ "$image_name" =~ ^ghcr.io/ ]]; then
    ...
    tags=$(curl -s "https://hub.docker.com/v2/repositories/${ns_repo}/tags?page_size=100" | jq -r '.results[].name')
fi

[[ "$DEBUG" == "true" ]] && echo "$tags" | tee -a "$LOG_FILE"

echo "$tags" | grep -E '^[vV]?[0-9]+(\.[0-9]+){1,2}(-[a-z0-9]+)?$' | grep -vE 'latest|dev|rc' | sort -Vr | head -n1 | tee "$cache_file"
...
commit_and_push() { cd "$REPO_DIR" git config user.email "updater@local" git config user.name "Add-on Updater"
