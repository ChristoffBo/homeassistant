...
send_notification() {
    ...
    case "${NOTIFICATION[service]}" in
        ...
        "apprise")
            ...
            ;;
        *)
            log "$COLOR_YELLOW" "⚠️ Unknown notification service: ${NOTIFICATION[service]}"
            ;;
    esac
}
...
# Function to get the latest Docker image tag
...
perform_update_check() {
    ...
    if [[ "$any_updates" -eq 1 && -n "$(git status --porcelain)" ]]; then
        ...
        if [[ "${CONFIG[skip_push]}" != "true" ]]; then
            ...
            log "$COLOR_CYAN" "⏸️ Skip push enabled - committed changes locally but not pushing"
        fi
    else
