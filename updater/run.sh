...
# Logging function with UTC timestamp, change TZ below if needed
log() {
  local color="$1"
  shift
  echo -e "$(date -u '+[%Y-%m-%d %H:%M:%S UTC]') ${color}$*${COLOR_RESET}" | tee -a "$LOG_FILE"
}
...
# Uncomment next line to use your configured timezone for date commands
# export TZ="$TIMEZONE"
...
update_addon_if_needed() {
  ...
  log "$COLOR_BLUE" "🚀 Checking latest version for image: $image"
  ...
  if [ "$latest_version" != "$current_version" ] && [ "$latest_version" != "latest" ]; then
    ...
    # Update or create CHANGELOG.md (prepend)
...
# Run cron daemon in foreground with logs redirected to LOG_FILE
exec crond -f -L /dev/stdout >> "$LOG_FILE" 2>&1
