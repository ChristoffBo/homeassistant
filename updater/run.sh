...
update_addon_if_needed() {
  ...
  # If upstream_version empty fallback to config.json version if present
  if [ -z "$upstream_version" ] && [ -f "$config_file" ]; then
    upstream_version=$(jq -r '.version // empty' "$config_file")
  fi
...
# Save last run date and time
echo "$(date '+%Y-%m-%d %H:%M:%S')" > "$LAST_RUN_FILE"

log "$COLOR_GREEN" "✅ Update completed at $(cat $LAST_RUN_FILE)"
...
while true; do
  ...
  if [ -f "$LAST_RUN_FILE" ]; then
    LAST_RUN=$(cut -d' ' -f1 "$LAST_RUN_FILE")
  fi
