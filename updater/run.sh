...
notify() { local title="$1" local message="$2" local priority="${3:-0}"

[[ "$NOTIFY_ENABLED" != "true" ]] && return case "$priority" in 0) [[ "$NOTIFY_SUCCESS" != "true" ]] && return ;; 3) [[ "$NOTIFY_UPDATES" != "true" ]] && return ;; 5) [[ "$NOTIFY_ERROR" != "true" ]] && return ;; esac

local payload=$(jq -n --arg t "$title" --arg m "$message" --argjson p "$priority" '{title: $t, message: $m, priority: $p}')

case "$NOTIFY_SERVICE" in gotify) curl -s -X POST "${NOTIFY_URL%/}/message?token=${NOTIFY_TOKEN}" 
-H "Content-Type: application/json" -d "$payload" >/dev/null ;; apprise|mailrise) curl -s -X POST "$NOTIFY_URL" 
-H 'Content-Type: application/json' 
-d "{"token":"$NOTIFY_TOKEN","to":"$NOTIFY_TO","title":"$title","message":"$message"}" >/dev/null ;; esac }

get_latest_tag() { local image="$1" [[ -z "$image" ]] && return

local image_name="${image%%:*}" local cache_file="/tmp/tags_$(echo "$image_name" | tr '/' '_').txt" local tags=""

if [[ "$image_name" =~ ^ghcr.io/ ]]; then local path="${image_name#ghcr.io/}" local org_repo="${path%%/}" local package="${path#/}" local token=$(curl -sf "https://ghcr.io/token?scope=repository:$org_repo/$package:pull" | jq -r '.token') tags=$(curl -sf -H "Authorization: Bearer $token" "https://ghcr.io/v2/$org_repo/$package/tags/list" | jq -r '.tags[]?') elif [[ "$image_name" =~ ^(linuxserver|lscr.io)/ ]]; then local name="${image_name##*/}" tags=$(curl -sf "https://fleet.linuxserver.io/api/v1/images/$name/tags" | jq -r '.tags[].name') else local ns_repo="${image_name/library//}" local page=1 while :; do local result=$(curl -sf "https://hub.docker.com/v2/repositories/${ns_repo}/tags?page=$page&page_size=100") || break local page_tags=$(echo "$result" | jq -r '.results[].name') [[ -z "$page_tags" ]] && break tags+=$'\n'$page_tags [[ "$(echo "$result" | jq -r '.next')" == "null" ]] && break ((page++)) done fi

echo "$tags" | grep -E '^[vV]?[0-9]+(.[0-9]+){1,2}(-[a-z0-9]+)?$' | grep -viE 'latest|dev|rc|beta' | sort -Vr | head -n1 | tee "$cache_file" }

update_addon() { local addon_path="$1" local name=$(basename "$addon_path")

[[ "$name" == "updater" || "$name" == "heimdall" ]] && return

log "$COLOR_DARK_BLUE" "🔍 Checking $name"

local config="$addon_path/config.json" local build="$addon_path/build.json" local image version latest

image=$(jq -r '.image // empty' "$config" 2>/dev/null) version=$(safe_jq '.version' "$config")

if [[ -z "$image" && -f "$build" ]]; then image=$(jq -r '.build_from.amd64 // .build_from | strings' "$build" 2>/dev/null) version=$(safe_jq '.version' "$build") fi

[[ -z "$image" ]] && { log "$COLOR_YELLOW" "⚠️ No image defined for $name" UNCHANGED_ADDONS["$name"]="No image" return }

latest=$(get_latest_tag "$image") [[ -z "$latest" ]] && { log "$COLOR_YELLOW" "⚠️ No valid version tag found for $image" UNCHANGED_ADDONS["$name"]="No valid tag" return }

if [[ "$version" != "$latest" ]]; then log "$COLOR_GREEN" "⬆️ $name updated from $version to $latest" UPDATED_ADDONS["$name"]="$version → $latest"
...
else log "$COLOR_CYAN" "✅ $name is up to date" UNCHANGED_ADDONS["$name"]="Up to date ($version)" fi }
...
main() { echo "" > "$LOG_FILE" read_config log "$COLOR_BLUE" "ℹ️ Starting Home Assistant Add-on Updater"

[[ -d "$REPO_DIR" ]] && rm -rf "$REPO_DIR" git clone --depth 1 "$GIT_AUTH_REPO" "$REPO_DIR" || { log "$COLOR_RED" "❌ Git clone failed" notify "Updater Error" "Git clone failed" 5 exit 1 }

for path in "$REPO_DIR"/*; do [[ -d "$path" ]] && update_addon "$path" done

commit_and_push

local summary="📦 Add-on Update Summary\n" summary+="🕒 $(date '+%Y-%m-%d %H:%M:%S %Z')\n\n" for name in "${!UPDATED_ADDONS[@]}"; do summary+="$name: 🔄 ${UPDATED_ADDONS[$name]}\n" done for name in "${!UNCHANGED_ADDONS[@]}"; do summary+="$name: ✅ ${UNCHANGED_ADDONS[$name]}\n" done [[ "$DRY_RUN" == "true" ]] && summary+="\n🔁 DRY RUN MODE ENABLED"

notify "Add-on Updater" "$summary" 3 log "$COLOR_BLUE" "ℹ️ Update process complete." }
