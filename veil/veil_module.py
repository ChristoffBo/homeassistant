...
async def query_doq(wire_query: bytes, server: str) -> Optional[bytes]:
    ...
    try:
        ...
        async with connect(
            ...
            client.send_stream_data(
                stream_id,
                struct.pack("!H", len(wire_query)) + wire_query,
                end_stream=True
            )
            client.transmit()
            ...
            while time.time() - start < CONFIG.get("upstream_timeout", 2.0):
                client.transmit()
            ...
            if expected_len and len(response_data) - 2 >= expected_len:
                ...
                response.id = 0
                response.flags |= dns.flags.RA | dns.flags.RD
