...
# ==================== STATISTICS ====================
STATS = {
    "dns_queries": 0, "dns_cached": 0, "dns_blocked": 0, "dns_upstream": 0,
    "dns_parallel": 0, "dns_padded": 0, "dns_0x20": 0, "dns_qname_min": 0,
    "dns_rate_limited": 0, "dns_safesearch": 0, "dns_dnssec_validated": 0,
    "dns_dnssec_failed": 0, "dns_doq_queries": 0, "dhcp_discovers": 0,
    "dhcp_offers": 0, "dhcp_requests": 0, "dhcp_acks": 0, "dhcp_naks": 0,
    "dhcp_declines": 0, "dhcp_releases": 0, "dhcp_informs": 0,
    "dhcp_ping_checks": 0, "dhcp_conflicts": 0, "blocklist_updates": 0,
    "blocklist_last_update": 0, "cache_prewarm_runs": 0, "cache_prewarm_last": 0,
    "cache_prewarm_domains": 0, "start_time": time.time()
}
...
class LRUCache:
    ...
    async def set(self, qname: str, qtype: int, response: bytes, ttl: int, negative: bool = False):
        ...
        now = time.time()
        expires = now + ttl
        stale_ttl = expires + (ttl * CONFIG.get("stale_ttl_multiplier", 2))
        async with self._lock:
            if len(self._cache) >= self._max_size:
                self._cache.popitem(last=False)
            self._cache[key] = CacheEntry(response=response, expires=expires, negative=negative, stale_ttl=stale_ttl)
            self._cache.move_to_end(key)
...
class RateLimiter:
    ...
    async def check_rate_limit(self, client_ip: str) -> bool:
        if not CONFIG.get("rate_limit_enabled"): return True
        ...
        while True:
            try:
                ...
                async with self._lock:
                    ...
                    for ip in stale: del self._clients[ip]
                    if stale: log.debug(f"[ratelimit] Cleaned {len(stale)} stale entries")
...
# ==================== SAFESEARCH ====================
SAFESEARCH_MAPPINGS = {
    "www.google.com": "forcesafesearch.google.com", "google.com": "forcesafesearch.google.com",
    ...
    "www.youtube.com": "restrict.youtube.com", "youtube.com": "restrict.youtube.com",
    "m.youtube.com": "restrict.youtube.com", "youtubei.googleapis.com": "restrict.youtube.com",
}

def apply_safesearch(qname: str) -> Optional[str]:
    if not CONFIG.get("safesearch_enabled"): return None
...
class DomainList:
    ...
    async def contains(self, domain: str) -> bool:
        ...
        async with self._lock:
            ...
            for part in parts:
                ...
                node = node.children[part]
                if node.is_blocked: return True
        return False
    ...
    async def remove(self, domain: str):
        ...
        async with self._lock:
            node = self._root
            for part in parts:
                if part not in node.children: return
                node = node.children[part]
            if node.is_blocked:
    ...
    async def export(self) -> List[str]:
        ...
        async def traverse(node, path):
            if node.is_blocked: domains.append('.'.join(reversed(path)))
    ...
    async def import_list(self, domains: List[str]):
        for domain in domains: self.add_sync(domain)
...
async def load_blocklists_local():
    try:
        ...
        with open(storage_path) as f:
            data = json.load(f)
        await BLOCKLIST.import_list(data.get("blocklist", []))
        await BLACKLIST.import_list(data.get("blacklist", []))
        await WHITELIST.import_list(data.get("whitelist", []))
        STATS["blocklist_last_update"] = data.get("last_update", 0)
        log.info(f"[storage] Loaded {len(data.get('blocklist', [])):,} blocklist, {len(data.get('blacklist', [])):,} blacklist, {len(data.get('whitelist', [])):,} whitelist domains from local storage")
        return True
    except Exception as e:
...
class BlocklistUpdater:
    ...
    async def update_blocklists(self):
        if not CONFIG.get("blocklist_update_enabled"): return
        log.info("[blocklist] Starting update")
        urls = CONFIG.get("blocklist_urls", [])
        if not urls: log.debug("[blocklist] No URLs configured"); return
        ...
        log.info(f"[blocklist] Update complete: {total_added:,} domains loaded, {BLOCKLIST.size:,} total")
   
    async def auto_update_loop(self):
        self.running = True
        if CONFIG.get("blocklist_update_on_start"): await self.update_blocklists()
        while self.running:
            try:
                await asyncio.sleep(CONFIG.get("blocklist_update_interval", 86400))
                if CONFIG.get("blocklist_update_enabled"): await self.update_blocklists()
            except asyncioCallCancelledError: break
            except Exception as e:
...
class CachePrewarmer:
    ...
    async def get_domains_to_prewarm(self) -> List[str]:
        ...
        if "popular" in sources: domains.update(POPULAR_DOMAINS)
        if "custom" in sources: domains.update(CONFIG.get("cache_prewarm_custom_domains", []))
    ...
    async def prewarm_domain(self, domain: str) -> bool:
        try:
            ...
            if response_a: log.debug(f"[prewarm] Cached A: {domain}")
            response_aaaa = await query_upstream(domain, dns.rdatatype.AAAA)
            if response_aaaa: log.debug(f"[prewarm] Cached AAAA: {domain}")
    ...
    async def prewarm_cache(self):
        ...
        if not domains: log.info("[prewarm] No domains to prewarm"); return
    ...
    async def auto_prewarm_loop(self):
        self.running = True
        if CONFIG.get("cache_prewarm_on_start"): await self.prewarm_cache()
        while self.running:
            try:
                await asyncio.sleep(CONFIG.get("cache_prewarm_interval", 3600))
                if CONFIG.get("cache_prewarm_enabled"): await self.prewarm_cache()
    ...
    def stop(self):
        self.running = False
        if self.prewarm_task: self.prewarm_task.cancel()
...
class UpstreamHealth:
    ...
    async def record_failure(self, server: str):
        async with self._lock:
            ...
            self._health[server]["last_check"] = time.time()
            if self._health[server]["failures"] >= CONFIG.get("upstream_max_failures", 3):
...
async def validate_dnssec(response_wire: bytes) -> bool:
    if not CONFIG.get("dnssec_validate"): return True
    try:
        response = dns.message.from_wire(response_wire)
        if not any(rrset.rdtype in (dns.rdatatype.RRSIG, dns.rdatatype.DNSKEY) for section in [response.answer, response.authority] for rrset in section):
            return True
...
async def query_doq(wire_query: bytes, server: str) -> Optional[bytes]:
    if not DOQ_AVAILABLE or not CONFIG.get("doq_enabled"): return None
    try:
        STATS["dns_doq_queries"] += 1
        configuration = QuicConfiguration(is_client=True, alpn_protocols=["doq"])
        async with connect(server, 853, configuration=configuration) as protocol:
...
async def get_conn_pool():
    global CONN_POOL
    if not CONN_POOL:
        CONN_POOL = ClientSession(connector=TCPConnector(limit=100, limit_per_host=10), timeout=ClientTimeout(total=CONFIG["upstream_timeout"]))
    return CONN_POOL

def apply_0x20_encoding(qname: str) -> str:
    if not CONFIG.get("case_randomization"): return qname
    STATS["dns_0x20"] += 1
    return ''.join(c.upper() if random.random() > 0.5 else c.lower() if c.isalpha() else c for c in qname)

def pad_query(wire_data: bytes) -> bytes:
    if not CONFIG.get("padding_enabled"): return wire_data
    STATS["dns_padded"] += 1
    block_size = CONFIG.get("padding_block_size", 468)
    current_len = len(wire_data)
    if current_len >= block_size: return wire_data
    padding_needed = block_size - (current_len % block_size)
    if padding_needed == block_size: padding_needed = 0
    try:
        msg = dns.message.from_wire(wire_data)
        if not msg.edns: msg.use_edns(edns=True, payload=4096)
        padded = msg.to_wire()
        if len(padded) < block_size:
            padded += b'\x00' * (block_size - len(padded))
        return padded
    except: return wire_data
...
async def query_upstream_parallel(qname: str, qtype: int) -> Optional[Tuple[bytes, str]]:
    ...
    if CONFIG.get("query_jitter"): query.id = random.randint(0, 65535)
    wire_query = query.to_wire()
    wire_query = pad_query(wire_query)
    if CONFIG.get("query_jitter"):
        jitter = random.randint(*CONFIG.get("query_jitter_ms", [10, 100])) / 1000.0
        await asyncio.sleep(jitter)
    tasks = []
    for server in servers:
        if CONFIG.get("doq_enabled") and DOQ_AVAILABLE:
            tasks.append((query_doq(wire_query, server), server))
        elif CONFIG.get("doh_enabled") and server in ["1.1.1.1", "1.0.0.1", "8.8.8.8", "8.8.4.4"]:
            tasks.append((query_doh(wire_query, server), server))
        elif CONFIG.get("dot_enabled"):
            tasks.append((query_dot(wire_query, server), server))
        else:
            tasks.append((query_udp(wire_query, server), server))
    STATS["dns_parallel"] += 1
    for coro, server in asyncio.as_completed([t[0] for t in tasks]):
        try:
            result = await coro
            if result:
                await UPSTREAM_HEALTH.record_success(server, time.time() - time.time())
...
async def query_dot(wire_query: bytes, server: str) -> Optional[bytes]:
    ...
    try:
        ...
        reader, writer = await asyncio.wait_for(asyncio.open_connection(server, 853, ssl=ssl_context), timeout=CONFIG["upstream_timeout"])
        msg_len = struct.pack('!H', len(wire_query))
...
def strip_ecs(response_wire: bytes) -> bytes:
    if not CONFIG.get("ecs_strip"): return response_wire
    try:
        response = dns.message.from_wire(response_wire)
        if response.edns >= 0 and hasattr(response, 'options'):
            original_len = len(response.options)
            response.options = [opt for opt in response.options if opt.otype != 8]
            if len(response.options) < original_len: STATS["dns_ecs_stripped"] += 1
        return response.to_wire()
    except: return response_wire
...
async def process_dns_query(data: bytes, addr: Tuple[str, int]) -> bytes:
    try:
        ...
        if safesearch_domain: qname = safesearch_domain
        if await WHITELIST.contains(qname): pass
        else:
        ...
        if qname in dns_rewrites: return build_rewrite_response(query, dns_rewrites[qname])
        local_records = CONFIG.get("local_records", {})
        if qname in local_records: return build_rewrite_response(query, local_records[qname])
        for domain, forward_server in CONFIG.get("conditional_forwards", {}).items():
            if qname.endswith(domain) or qname == domain.strip('.'):
                try:
                    ...
                    if response_wire: return response_wire
                except Exception as e:
        ...
        if CONFIG.get("rebinding_protection"):
            rebinding_exempt = any(qname.endswith(exempt) or qname == exempt.strip('.') for exempt in CONFIG.get("rebinding_whitelist", []))
        ...
        if response.answer:
            ttl = min(rrset.ttl for rrset in response.answer) or ttl
        else:
...
class DNSProtocol(asyncio.DatagramProtocol):
    ...
    async def handle_query(self, data, addr):
        try:
            ...
            if response: self.transport.sendto(response, addr)
            else: log.warning(f"[dns] No response generated for query from {addr[0]}")
        except Exception as e:
...
async def start_dns():
    ...
    try:
        ...
        loop = asyncio.get_event_loop()
        transport, protocol = await loop.create_datagram_endpoint(lambda: DNSProtocol(), local_addr=(CONFIG["dns_bind"], CONFIG["dns_port"]), reuse_port=True)
        dns_transport = transport
        log.info(f"[dns] SUCCESSFULLY started on {CONFIG['dns_bind']}:{CONFIG['dns_port']}")
...
class DHCPLease:
    ...
    def to_dict(self) -> dict:
        return {"mac": self.mac, "ip": self.ip, "hostname": self.hostname, "client_id": self.client_id, "lease_start": self.lease_start, "lease_end": self.lease_end, "expires_in": max(0, int(self.lease_end - time.time())), "static": self.static}
...
class DHCPServer:
    ...
    def _init_ip_pool(self):
        ...
        self.ip_pool = [str(ipaddress.IPv4Address(ip)) for ip in range(int(start_ip), int(end_ip) + 1)]
    ...
    def _load_leases(self):
        ...
        for mac, ip in CONFIG.get("dhcp_static_leases", {}).items():
            self.leases[mac] = DHCPLease(mac=mac, ip=ip, static=True, lease_start=time.time(), lease_end=time.time() + (365 * 86400))
    ...
    async def _ping_check(self, ip: str) -> bool:
        if not CONFIG.get("dhcp_ping_check"): return False
        STATS["dhcp_ping_checks"] += 1
        try:
            proc = await asyncio.create_subprocess_exec("ping", "-c", "1", "-W", str(CONFIG.get("dhcp_ping_timeout", 1)), ip, stdout=asyncio.subprocess.DEVNULL, stderr=asyncio.subprocess.DEVNULL)
    ...
    async def _get_available_ip(self, mac: str) -> Optional[str]:
        async with self.lock:
            if mac in self.leases and not self.leases[mac].is_expired(): return self.leases[mac].ip
            used_ips = {lease.ip for lease in self.leases.values() if not lease.is_expired()}
            for ip in self.ip_pool:
                if ip not in used_ips:
                    if await self._ping_check(ip): continue
                    return ip
        return None
    def _parse_dhcp_packet(self, data: bytes) -> Optional[dict]:
        ...
        try:
            packet = {
                ...
                "xid": struct.unpack("!I", data[4:8])[0], "secs": struct.unpack("!H", data[8:10])[0],
                "flags": struct.unpack("!H", data[10:12])[0], "ciaddr": socket.inet_ntoa(data[12:16]),
                "yiaddr": socket.inet_ntoa(data[16:20]), "siaddr": socket.inet_ntoa(data[20:24]),
                "giaddr": socket.inet_ntoa(data[24:28]), "chaddr": ':'.join(f'{b:02x}' for b in data[28:34]),
                "sname": data[44:108].split(b'\x00')[0].decode('utf-8', errors='ignore'),
                "file": data[108:236].split(b'\x00')[0].decode('utf-8', errors='ignore'), "options": {}
            }
            if data[236:240] != b'\x63\x82\x53\x63': return None
            i = 240
            while i < len(data):
                opt = data[i]
                if opt == DHCP_OPT_END: break
                if opt == DHCP_OPT_PAD: i += 1; continue
                if i + 1 >= len(data): break
                opt_len = data[i + 1]
                if i + 2 + opt_len > len(data): break
    ...
    def _build_dhcp_packet(self, packet: dict, msg_type: int, offered_ip: str) -> bytes:
        ...
        response[0] = 2; response[1] = packet["htype"]; response[2] = packet["hlen"]; response[3] = 0
        ...
        pos = 240
        response[pos:pos + 3] = bytes([DHCP_OPT_MESSAGE_TYPE, 1, msg_type]); pos += 3
        server_ip = socket.inet_aton(CONFIG["dhcp_gateway"])
        response[pos:pos + 6] = bytes([DHCP_OPT_SERVER_ID, 4]) + server_ip; pos += 6
        lease_time = CONFIG["dhcp_lease_time"]
        response[pos:pos + 6] = bytes([DHCP_OPT_LEASE_TIME, 4]) + struct.pack("!I", lease_time); pos += 6
        renewal_time = CONFIG.get("dhcp_renewal_time", lease_time // 2)
        response[pos:pos + 6] = bytes([DHCP_OPT_RENEWAL_TIME, 4]) + struct.pack("!I", renewal_time); pos += 6
        rebinding_time = CONFIG.get("dhcp_rebinding_time", int(lease_time * 0.875))
        response[pos:pos + 6] = bytes([DHCP_OPT_REBINDING_TIME, 4]) + struct.pack("!I", rebinding_time); pos += 6
        netmask = socket.inet_aton(CONFIG["dhcp_netmask"])
        response[pos:pos + 6] = bytes([DHCP_OPT_SUBNET_MASK, 4]) + netmask; pos += 6
        gateway = socket.inet_aton(CONFIG["dhcp_gateway"])
        response[pos:pos + 6] = bytes([DHCP_OPT_ROUTER, 4]) + gateway; pos += 6
        try:
            ...
            response[pos:pos + 6] = bytes([DHCP_OPT_BROADCAST, 4]) + broadcast; pos += 6
        except: pass
        ...
        response[pos:pos + 2 + len(dns_bytes)] = bytes([DHCP_OPT_DNS_SERVER, len(dns_bytes)]) + dns_bytes; pos += 2 + len(dns_bytes)
        if CONFIG.get("dhcp_domain"):
            domain = CONFIG["dhcp_domain"].encode()
            response[pos:pos + 2 + len(domain)] = bytes([DHCP_OPT_DOMAIN_NAME, len(domain)]) + domain; pos += 2 + len(domain)
        if CONFIG.get("dhcp_ntp_servers"):
            ...
            response[pos:pos + 2 + len(ntp_bytes)] = bytes([DHCP_OPT_NTP_SERVER, len(ntp_bytes)]) + ntp_bytes; pos += 2 + len(ntp_bytes)
        if CONFIG.get("dhcp_wins_servers"):
            ...
            response[pos:pos + 2 + len(wins_bytes)] = bytes([DHCP_OPT_WINS_SERVER, len(wins_bytes)]) + wins_bytes; pos += 2 + len(wins_bytes)
        ...
        if vendor_opts and isinstance(vendor_opts, dict):
            ...
            for opt_code, opt_value in vendor_opts.items():
                if isinstance(opt_value, str): opt_value = opt_value.encode()
                vendor_data += bytes([int(opt_code), len(opt_value)]) + opt_value
            if vendor_data:
                response[pos:pos + 2 + len(vendor_data)] = bytes([DHCP_OPT_VENDOR_SPECIFIC, len(vendor_data)]) + vendor_data; pos += 2 + len(vendor_data)
        if CONFIG.get("dhcp_tftp_server"):
            tftp = CONFIG["dhcp_tftp_server"].encode()
            response[pos:pos + 2 + len(tftp)] = bytes([DHCP_OPT_TFTP_SERVER, len(tftp)]) + tftp; pos += 2 + len(tftp)
        if CONFIG.get("dhcp_bootfile"):
            bootfile = CONFIG["dhcp_bootfile"].encode()
            response[pos:pos + 2 + len(bootfile)] = bytes([DHCP_OPT_BOOTFILE, len(bootfile)]) + bootfile; pos += 2 + len(bootfile)
    ...
    async def handle_discover(self, packet: dict, addr: Tuple[str, int]):
        ...
        if not offered_ip: log.warning(f"[dhcp] No available IP for {mac}"); return
        log.info(f"[dhcp] DISCOVER from {mac} -> offering {offered_ip}")
        STATS["dhcp_discovers"] += 1; STATS["dhcp_offers"] += 1
        ...
        if packet["flags"] & 0x8000:
            ...
            try: self.sock.sendto(response, (offered_ip, 68))
            except: self.sock.sendto(response, ('<broadcast>', 68))
    async def handle_request(self, packet: dict, addr: Tuple[str, int]):
        ...
        if not requested_ip: log.warning(f"[dhcp] REQUEST from {mac} without requested IP"); return
        ...
        if requested_ip not in self.ip_pool and requested_ip != available_ip:
            ...
            STATS["dhcp_naks"] += 1
            return
        hostname = packet["options"].get(DHCP_OPT_HOSTNAME, b'').decode('utf-8', errors='ignore')
        client_id = packet["options"].get(DHCP_OPT_CLIENT_ID, b'').hex()
        async with self.lock:
            self.leases[mac] = DHCPLease(mac=mac, ip=requested_ip, hostname=hostname, client_id=client_id, lease_start=time.time(), lease_end=time.time() + CONFIG["dhcp_lease_time"], static=mac in CONFIG.get("dhcp_static_leases", {}))
            self._save_leases()
        ...
        if packet["flags"] & 0x8000:
            ...
            try: self.sock.sendto(response, (requested_ip, 68))
            except: self.sock.sendto(response, ('<broadcast>', 68))
    async def handle_decline(self, packet: dict, addr: Tuple[str, int]):
        mac = packet["chaddr"]
        declined_ip = socket.inet_ntoa(packet["options"].get(DHCP_OPT_REQUESTED_IP, b'\x00\x00\x00\x00'))
        log.warning(f"[dhcp] DECLINE from {mac} for {declined_ip} - IP conflict detected")
        STATS["dhcp_declines"] += 1; STATS["dhcp_conflicts"] += 1
        async with self.lock:
    ...
    async def handle_packet(self, data: bytes, addr: Tuple[str, int]):
        try:
            ...
            if DHCP_OPT_MESSAGE_TYPE not in packet["options"]: return
            msg_type = packet["options"][DHCP_OPT_MESSAGE_TYPE][0]
            if msg_type == DHCP_DISCOVER: await self.handle_discover(packet, addr)
            elif msg_type == DHCP_REQUEST: await self.handle_request(packet, addr)
            elif msg_type == DHCP_DECLINE: await self.handle_decline(packet, addr)
            elif msg_type == DHCP_RELEASE: await self.handle_release(packet, addr)
            elif msg_type == DHCP_INFORM: await self.handle_inform(packet, addr)
        except Exception as e:
    ...
    async def cleanup_expired_leases(self):
        while self.running:
            try:
                await asyncio.sleep(300)
                async with self.lock:
                    expired = [mac for mac, lease in self.leases.items() if lease.is_expired() and not lease.static]
                    for mac in expired:
                        log.info(f"[dhcp] Lease expired: {mac} -> {self.leases[mac].ip}")
                        del self.leases[mac]
                    if expired: self._save_leases()
            except Exception as e:
    ...
    async def _receive_loop(self):
        ...
        while self.running:
            try:
                ...
                if self.running: log.error(f"[dhcp] Receive error: {e}")
    ...
    def add_static_lease(self, mac: str, ip: str, hostname: str = "") -> bool:
        if ip not in self.ip_pool: return False
        self.leases[mac] = DHCPLease(mac=mac, ip=ip, hostname=hostname, static=True, lease_start=time.time(), lease_end=time.time() + (365 * 86400))
        if "dhcp_static_leases" not in CONFIG: CONFIG["dhcp_static_leases"] = {}
...
async def api_stats(req):
    ...
    for server in CONFIG.get("upstream_servers", []):
        if server not in upstream_health:
            upstream_health[server] = {"healthy": True, "success_rate": 1.0, "total": 0, "success": 0, "avg_latency": 0}
    return web.json_response({
        "total_queries": STATS.get("dns_queries", 0), "blocked": STATS.get("dns_blocked", 0), "cache_hits": STATS.get("dns_cached", 0),
        "upstream_queries": STATS.get("dns_upstream", 0), "padded": STATS.get("dns_padded", 0), "ecs_stripped": STATS.get("dns_ecs_stripped", 0),
        "case_randomized": STATS.get("dns_0x20", 0), "dnssec_valid": STATS.get("dns_dnssec_validated", 0),
        "dhcp_offers": STATS.get("dhcp_offers", 0), "dhcp_acks": STATS.get("dhcp_acks", 0),
        "active_leases": len(DHCP_SERVER.leases) if DHCP_SERVER else 0, "pool_total": len(DHCP_SERVER.ip_pool) if DHCP_SERVER else 0,
        "uptime_seconds": uptime, "dns_running": dns_transport is not None, "dhcp_running": DHCP_SERVER.running if DHCP_SERVER else False,
        "queries_per_second": STATS.get("dns_queries", 0) / max(uptime, 1), "cache_size": DNS_CACHE.size(),
        "blocklist_size": BLOCKLIST.size, "whitelist_size": WHITELIST.size, "blacklist_size": BLACKLIST.size,
        "upstream_health": upstream_health, "leases": [lease.to_dict() for lease in DHCP_SERVER.leases.values()] if DHCP_SERVER else [],
        **STATS
    })

async def api_config_get(req): return web.json_response(CONFIG)
async def api_config_update(req):
    try:
        data = await req.json()
        if "upstream_servers" in data:
            servers = []
            doh_map = {"cloudflare-dns.com": ["1.1.1.1", "1.0.0.1"], "dns.google": ["8.8.8.8", "8.8.4.4"]}
            for server in data["upstream_servers"]:
                server = server.strip()
                if not server: continue
                if server.startswith("http"):
                    for domain, ips in doh_map.items():
                        if domain in server: servers.extend(ips); break
                else: servers.append(server)
            data["upstream_servers"] = list(set(servers))
        for key, value in data.items():
            if key in CONFIG: CONFIG[key] = value
        try:
            with open('/config/options.json', 'r') as f: file_config = json.load(f)
            file_config.update(data)
            with open('/config/options.json', 'w') as f: json.dump(file_config, f, indent=2)
            log.info(f"[api] Config saved to /config/options.json")
        except Exception as e: log.warning(f"[api] Could not save config to file: {e}")
        ...
        for bl in CONFIG["blocklists"]:
            if Path(bl).exists():
                with open(bl) as f:
                    for line in f:
                        line = line.strip()
                        if line and not line.startswith('#'): BLOCKLIST.add_sync(line)
        await save_blocklists_local()
...
async def api_blocklist_update(req):
    try:
        ...
        return web.json_response({"status": "updated", "size": BLOCKLIST.size, "last_update": STATS["blocklist_last_update"]})
    except Exception as e:
        return web.json_response({"error": str(e)}, status=400)
...
async def api_cache_prewarm(req):
    try:
        ...
        return web.json_response({"status": "started", "message": "Cache prewarm started in background"})
    except Exception as e:
        return web.json_response({"error": str(e)}, status=400)
...
async def api_query_history(req):
    try:
        ...
        return web.json_response({"total_unique": QUERY_HISTORY.size(), "top_domains": top_domains})
    except Exception as e:
        return web.json_response({"error": str(e)}, status=400)
...
async def api_blocklist_upload(req):
    try:
        data = await req.post()
        if 'file' not in data: return web.json_response({"error": "No file provided"}, status=400)
...
async def api_rewrite_add(req):
    try:
        ...
        if not domain or not value: return web.json_response({"error": "Domain and value required"}, status=400)
        if "dns_rewrites" not in CONFIG: CONFIG["dns_rewrites"] = {}
        CONFIG["dns_rewrites"][domain] = {"type": record_type, "value": value, "ttl": ttl}
        return web.json_response({"status": "added", "domain": domain})
    except Exception as e:
        return web.json_response({"error": str(e)}, status=400)
...
async def api_local_record_add(req):
    try:
        ...
        ttl = int(data.get('ttl', 300))
        if not domain or not value: return web.json_response({"error": "Domain and value required"}, status=400)
        if "local_records" not in CONFIG: CONFIG["local_records"] = {}
        CONFIG["local_records"][domain] = {"type": record_type, "value": value, "ttl": ttl}
        return web.json_response({"status": "added", "domain": domain})
    except Exception as e:
        return web.json_response({"error": str(e)}, status=400)
...
async def api_dhcp_leases(req):
    if not DHCP_SERVER: return web.json_response({"error": "DHCP not enabled"}, status=400)
...
async def api_dhcp_static_add(req):
    try:
        ...
        if not mac or not ip: return web.json_response({"error": "MAC and IP required"}, status=400)
...
async def init_veil():
    ...
    RATE_LIMITER.start()
    if CONFIG.get("cache_prewarm_enabled"): CACHE_PREWARMER.start()
    if CONFIG.get("blocklist_update_enabled"): BLOCKLIST_UPDATER.start()
...
async def cleanup_veil():
    ...
    await save_blocklists_local()
    if dns_transport: dns_transport.close()
    if DHCP_SERVER: DHCP_SERVER.stop()
    if CONN_POOL: await CONN_POOL.close()
...
if __name__ == "__main__":
    ...
    try:
        if os.path.exists(cfg_path):
            with open(cfg_path, "r") as f:
                data = json.load(f)
                if "dns_enabled" in data: data["enabled"] = data["dns_enabled"]
                if "dns_port_tcp" in data: data["dns_port"] = data["dns_port_tcp"]
                if "bind_address" in data: data["dns_bind"] = data["bind_address"]
                if "ui_bind" in data: data["ui_bind"] = data["ui_bind"]
                if "ui_port" in data: data["ui_port"] = data["ui_port"]
                CONFIG.update(data)
    ...
    if not os.path.exists(ui_path): ui_path = "/app/ui"
    if not os.path.exists(ui_path):
        async def placeholder(_): return web.Response(text="Veil is running, but no UI files found.")
        app.router.add_get("/", placeholder)
    else:
        app.router.add_static("/", ui_path, show_index=True)
        log.info(f"[veil] Serving UI from {ui_path}")
    app.on_startup.append(lambda app: init_veil())
    app.on_cleanup.append(lambda app: cleanup_veil())
